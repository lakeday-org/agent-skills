#!/usr/bin/env node
// GENERATED by scripts/build-plugins.mjs; hook sources 13fe73248e111aae
...
// node_modules/@lakeday-org/worker-js/src/learning.js
...
var RECORDING_OPERATIONS = /* @__PURE__ */ new Set([
  "decide",
  "outcome",
  "fact",
  "link",
...
function knowledgeOperation(lakedayTool) {
  const match = /^(?:entity|session)_(.+)$/.exec(lakedayTool ?? "");
  return match && RECORDING_OPERATIONS.has(match[1]) ? match[1] : null;
}
var RECORDING_TOOLS = new Set(
  ["entity", "session"].flatMap(
    (namespace) => [...RECORDING_OPERATIONS].map((operation) => `${namespace}_${operation}`)
  )
);
...
function aboutFor(tool) {
  const t = tool.lakedayTool;
  const input = tool.input ?? {};
  if ((t === "deploy_pipeline" || t === "run_pipeline") && input.name)
    return `pipeline:${sanitizeId(input.name)}`;
  if ((t === "deploy_worker" || t === "delete_worker" || t === "invoke_worker") && input.worker)
...
function stopVerdict(turn, {
  ...
  if (needsDecision) {
    ...
    lines.push(
      ...
      surface === "agent" ? `Call session_decide with {id: <short-kebab record id>, object: <the object the decision is about>, choice: <what you did>, rationale: <why>, alternatives: [<what you rejected>], evidence: [{object: <table or object id>, point: <version or record if known>}]}${sessionId ? ` (the current session is ${sessionId})` : ""}.` : `Call entity_decide with id set to the object the decision is about${projectEntity ? ` (${projectEntity} unless something else fits better)` : ""}, and data {id: <short-kebab record id>, actor_id, choice: <what you did>, rationale: <why>, alternatives: [<what you rejected>], evidence: [{object: <table or object id>, point: <version or record if known>}]}. data.id names the decision, never the session${sessionId ? `; set about to ${sessionId} so it points back at this session` : ""}.`,
      "A decision is recorded on the object it is about. Use one decision per independent choice. Then finish your reply."
    );
  }
  if (needsOutcome)
    lines.push(
      surface === "agent" ? `A prior decision${sessionId ? ` observed in session ${sessionId}` : ""} has no measured outcome, and this turn measured results. Call session_outcome with {record: <the decision's record id>, object: <the object it was recorded on>, outcome: <measured result>, evidence: [...]}.` : `A prior decision${sessionId ? ` observed in session ${sessionId}` : ""} has no measured outcome, and this turn measured results. Call entity_outcome with id set to the object the decision was recorded on, record_id set to the decision's record id, and data {actor_id, outcome: <measured result>, evidence: [{object, point}]}.`
    );
  lines.push(
  ...
  switch (normalized.event) {
    ...
    case "Stop": {
      const verdict = stopVerdict(state.turn, {
        ...
        projectEntity: state.projectEntity ?? void 0,
