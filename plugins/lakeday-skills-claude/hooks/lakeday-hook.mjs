#!/usr/bin/env node
// GENERATED by scripts/build-plugins.mjs; hook sources ef30e9317a6870d9
...
function stopVerdict(turn, {
  ...
  if (needsDecision) {
    ...
    lines.push(
      ...
      `Call session_decide${sessionId ? ` with id="${sessionId}"` : " with id set to the session"}, and pass data {id, subjects, choice, rationale, alternatives, evidence}. data.id is a short kebab-case name for the decision itself, such as restore-timeout: it is not the session id. subjects are the entity ids affected, evidence cites datasets with versions.`,
